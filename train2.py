...
col_train = ['rr', 'rr5', 'rr10', 'rr20', 'rr30', 'volumn_ratio']
col_label = ['rr-1']
...
trading_day = 1
loss_weight = [1, 1]
ped_weight = torch.ones(len(rr_num)).to(device)
flag = ['train', 'test', 'val']


# load data
data_set = EOD_data(root_path=data_path, market=market_name, PreConcept=PreConcept, flag=flag[1], tickers=tickers,
                    col_train=col_train, col_label=col_label, seq_len=seq_len, trading_day=trading_day, rr_num=rr_num)
...
# load model
model = HGAT(d_model=d_model, ecod_in=ecod_in, seq_len=seq_len, rr_num=rr_num, use_PreAttn=use_PreAttn,
             use_HidAttn=use_HidAttn, Hedge_index=inci_mat[1], device=device, encoder=en_model).to(device)
...
scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer_hgat, gamma=0.1, milestones=[40, 80])
scheduler2 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_hgat, mode='min', factor=0.1, patience=5,
                                                        threshold=1e-4)
scheduler3 = torch.optim.lr_scheduler.StepLR(optimizer_hgat, step_size=40, gamma=0.1, last_epoch=-1)
...
for epoch in loop1:
    ...
    end_time = time.time()
    pd_output = pd.concat([pd_output, pd.DataFrame([[train_loss, val_loss, val_per['roi'], val_per['sharp'], val_per['rank_score']]], columns=pd_col)], ignore_index=True)
...
with torch.no_grad():
    ...
    pd_output = pd.concat([pd_output, pd.DataFrame([[0, 0, test_per['roi'], test_per['sharp'], test_per['rank_score']]], columns=pd_col)], ignore_index=True)
...
if compare:
    ...
    pd_output.to_csv('./output/' + market_name + '_' + output_name + '_output9.csv')
